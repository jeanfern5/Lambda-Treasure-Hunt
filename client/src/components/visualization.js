...
import { LineSeries, MarkSeries, XYPlot } from 'react-vis';
 ...
 export default class Visualization extends Component {
  ...
  createNodes = () => {
    ...
    let coords = [];
    for (let room in map) {
      coords.push(map[room][0]);
      // console.log('HERE-------------->', map[room][0])
    }
    this.setState({ coordinates: coords });
    // console.log('HERE-------------->', Object.keys(coords))
  ...
  createEdges = () => {
    ...
    for(let room in map){
      for(let adjacentroom in map[room][1]){
          // console.log('HERE1-------------->', map[room][1])
          // console.log('HERE2-------------->', map[room][0])
          // console.log('HERE3-------------->', map[map[room]])
          if(typeof(map[map[room][1][adjacentroom]]) === 'undefined'){
            //TODO
          }
          else{
              connections.push([map[room][0], map[map[room][1][adjacentroom]][0]]);
          };
      };
    };
    this.setState({ lines: connections })
  };
  ...
  render(){
    return(
      <MapContainer className='map'>
        <button onClick={this.createMap}>CLICK HERE TO SEE MAP</button>   
        <XYPlot width={700} height={700}>
          {this.state.lines.map(connection => 
              <LineSeries strokeWidth='2'color='black' data={connection}/>
          )} 
          <MarkSeries
